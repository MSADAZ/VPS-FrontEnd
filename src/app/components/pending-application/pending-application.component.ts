...
export class PendingApplicationComponent implements OnInit {
  ...
  displayedColumns: string[] = ['id', 'name', 'processID', 'priority','actions'];
...
function createNewUser(id: number): UserData {
  ...
  return {
    id: id.toString(),
    name: name,
    processID: Math.round(Math.random() * 100).toString(),
    priority: COLORS[Math.round(Math.random() * (COLORS.length - 1))],
    actions:null
  };
}